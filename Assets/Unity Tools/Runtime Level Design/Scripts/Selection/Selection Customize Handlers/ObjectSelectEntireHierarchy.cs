    ...
    public class ObjectSelectEntireHierarchy : Singleton<ObjectSelectEntireHierarchy>
    {
        ...
        private void OnPreSelectCustomize(ObjectPreSelectCustomizeInfo customizeInfo, List<GameObject> toBeSelected)
        {
            if (IgnoreObjectGroups)
            {
                ...
                List<GameObject> selectThese = new List<GameObject>(roots.Count);
                foreach (var root in roots)
                {
                    var allChildrenAndSelf = root.GetAllChildrenAndSelf();
                    foreach(var go in allChildrenAndSelf)
                    {
                        if (RTObjectSelection.Get.CanSelectObject(go, RTObjectSelection.SelectRestrictFlags.All, ObjectSelectReason.PreSelectCustomize))
                            selectThese.Add(go);
                    }
                }
            ...
            else
            {
                ...
                foreach(var gameObj in toBeSelected)
                {
                    ...
                    {
                        if (RTObjectSelection.Get.CanSelectObject(hierarchyObject, RTObjectSelection.SelectRestrictFlags.All, ObjectSelectReason.PreSelectCustomize))
