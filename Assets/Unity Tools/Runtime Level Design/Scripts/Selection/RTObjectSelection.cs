...
namespace RLD
{
    ...
    public class RTObjectSelection : MonoSingleton<RTObjectSelection>
    {
        ...
        private ObjectBounds.QueryConfig _boundsQConfig = ObjectBounds.DefaultQConfig;
        ...
        public GameObject Object2ObjectSnapSessionSurfaceObject { get { return _object2ObjectSnapSession.SurfaceObject; } }
        public List<GameObject> SelectedObjects { get { return new List<GameObject>(_selectedObjects); } }
        public bool UseGrabBeginEndHotkyes { get; set; }
        ...
        public void Initialize_SystemCall()
        {
            ...
            _boundsQConfig.NoVolumeSize = Vector3Ex.FromValue(0.0f);   // We don't want to render anything for objects with no volume
            _boundsQConfig.ObjectTypes = GameObjectType.Mesh | GameObjectType.Sprite | GameObjectType.Terrain;
            _boundsQConfig.LayerMask = Settings.SelectableLayers;
            _boundsQConfig.CustomFilter = new Func<GameObject, bool>((GameObject go) => { return Settings.IsObjectSelectable(go); });
        }

        public void BeginObjectToObjectSnapSession()
        {
            if (ActiveManipSession == ObjectSelectionManipSession.Object2ObjectSnap) return;

            _grabSession.End();
            _gridSnapSession.End();
            _object2ObjectSnapSession.End();

            _object2ObjectSnapSession.Begin(_selectedObjects);
        }

        public void SetObjectToObjectSnapMoreControlEnabled(bool enabled)
        {
            _object2ObjectSnapSession.EnableMoreControl = enabled;
        }
        ...
        public void Delete()
        {
            ...
            _willBeDeleted = true;
            if (WillBeDeleted != null) WillBeDeleted(objectsToDelete);
        ...
        public AABB GetWorldAABB()
        {
            ...
            //boundsQConfig.ObjectTypes = GameObjectTypeHelper.AllCombined;
            return ObjectBounds.CalcObjectCollectionWorldAABB(_selectedObjects, _boundsQConfig);
        }

        public void SetGrabSessionEnabled(bool enabled)
        {
            if (UseGrabBeginEndHotkyes) return;

            if (enabled) _grabSession.Begin(_selectedObjects);
            else
            {
                _grabSession.End();
            }
        ...
        public void Update_SystemCall()
        {
            ...
            bool wasManipSessionActive = IsManipSessionActive;
            _multiSelectShape.MinSize = Settings.MinMultiSelectSize;
            if (!_multiSelectShape.IsVisible)
            {
                if (!IsManipSessionActive || IsGrabSessionActive) _grabSession.Update(_selectedObjects, UseGrabBeginEndHotkyes);
        ...
        public void Render_SystemCall(Camera renderCamera)
        {
            ...
            if (LookAndFeel.DrawHighlight && !IsManipSessionActive)
            {
                ...
                if (LookAndFeel.SelBoxRenderMode != SelectionBoxRenderMode.SelectionVolume)
                {
                    ...
                    {
                        OBB worldOBB = drawFromParentToBottom ? ObjectBounds.CalcHierarchyWorldOBB(selectedObject, _boundsQConfig) : ObjectBounds.CalcWorldOBB(selectedObject, _boundsQConfig);
                        if (worldOBB.IsValid)
                        {
                ...
                else
                {
                    ...
                    foreach (var selectedObject in _selectedObjects)
                    {
                        AABB worldAABB = ObjectBounds.CalcWorldAABB(selectedObject, _boundsQConfig);
        ...
        public bool CanSelectObject(GameObject gameObject, SelectRestrictFlags restrictFlags, ObjectSelectReason selectReason)
        {
        ...
        private void PerformClickSelect()
        {
            ...
            // Store the state of the selection before it is about to change. We will need this
