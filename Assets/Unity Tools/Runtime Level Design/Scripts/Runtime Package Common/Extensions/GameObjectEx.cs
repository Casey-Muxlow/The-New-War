...
namespace RLD
{
    public static class GameObjectEx
    {
        ...
        public static void InheritHierarchyLayers(this GameObject hierarchy, GameObject sourceHierarchy)
        {
            ...
            transformStack.Push(hierarchy.transform);
            Stack<Transform> srcTransformStack = new Stack<Transform>();
            srcTransformStack.Push(sourceHierarchy.transform);

            while (transformStack.Count != 0)
            {
                Transform transform = transformStack.Pop();
                Transform srcTransform = srcTransformStack.Pop();

                transform.gameObject.layer = srcTransform.gameObject.layer;
                for (int childIndex = 0; childIndex < transform.childCount; ++childIndex)
                {
                    transformStack.Push(transform.GetChild(childIndex));
                    srcTransformStack.Push(srcTransform.GetChild(childIndex));
                }
            }
        }
