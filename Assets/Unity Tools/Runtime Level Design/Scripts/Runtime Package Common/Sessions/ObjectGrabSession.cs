        ...
        public void Render(Camera renderCamera)
        {
            ...
            if (IsActive && _grabSurfaceInfo.SurfaceType != GrabSurfaceType.Invalid)
            {
                ...
                if (SharedLookAndFeel.DrawObjectPosTicks)
                {
                    ...
                    foreach (var grabTarget in _grabTargets)
                    {
                        Vector2 screenPos = renderCamera.WorldToScreenPoint(grabTarget.Transform.position);
                        GLRenderer.DrawRect2D(RectEx.FromCenterAndSize(screenPos, Vector2Ex.FromValue(SharedLookAndFeel.ObjectPosTickSize)), renderCamera);
                    }
                }
                ...
                if (SharedLookAndFeel.DrawAnchorPosTick)
                {
                    ...
                    Vector2 anchorScreenPos = renderCamera.WorldToScreenPoint(_grabSurfaceInfo.AnchorPoint);
                    GLRenderer.DrawRect2D(RectEx.FromCenterAndSize(anchorScreenPos, Vector2Ex.FromValue(SharedLookAndFeel.AnchorPosTickSize)), renderCamera);
                }
            }
        }

        public void Update(IEnumerable<GameObject> targetObjects, bool useBeginEndHotkeys)
        {
            if (SharedHotkeys == null || SharedSettings == null) return;

            if (useBeginEndHotkeys)
            {
