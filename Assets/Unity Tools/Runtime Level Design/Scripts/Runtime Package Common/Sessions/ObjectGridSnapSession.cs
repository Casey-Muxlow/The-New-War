    ...
    public class ObjectGridSnapSession
    {
        ...
        public void Render(Camera renderCamera)
        {
            ...
            if(IsActive)
            {
                ...
                Vector2 screenSnapPivot = renderCamera.WorldToScreenPoint(_snapPivotPoint);
                if (_sharedLookAndFeel.PivotShapeType == PivotPointShapeType.Circle)
                {
                    ...
                    GLRenderer.DrawTriangleFan2D(screenSnapPivot, pivotCirclePoints, renderCamera);

                    if(_sharedLookAndFeel.DrawPivotBorder)
                    {
                        material.SetColor(_sharedLookAndFeel.PivotPointBorderColor);
                        material.SetPass(0);
                        GLRenderer.DrawLineLoop2D(pivotCirclePoints, renderCamera);
                    }
                }
                else
                if(_sharedLookAndFeel.PivotShapeType == PivotPointShapeType.Square)
                {
                    ...
                    GLRenderer.DrawRect2D(pivotRect, renderCamera);

                    if (_sharedLookAndFeel.DrawPivotBorder)
                    {
                        material.SetColor(_sharedLookAndFeel.PivotPointBorderColor);
                        material.SetPass(0);
                        GLRenderer.DrawRectBorder2D(pivotRect, renderCamera);
                    }
                }
            }
        }
